...
test("_configureAndValidatePaths: Custom src directory", async (t) => {
	...
	const projectInput = {
		...
		configuration: {
			...
			resources: {
				configuration: {
					paths: {
						src: "webapp-properties.componentName"
					}
				}
			}
		}
	...
	t.is(project._srcPath, "webapp-properties.componentName", "Correct path for src");
});
...
test("namespace: detect namespace from pom.xml via ${project.artifactId}", async (t) => {
	...
	componentHInput.configuration.resources.configuration.paths.src = "webapp-project.artifactId";
...
test("namespace: detect namespace from pom.xml via ${componentName} from properties", async (t) => {
	const {componentHInput} = t.context;
	componentHInput.configuration.resources.configuration.paths.src = "webapp-properties.componentName";
	const project = await Specification.create(componentHInput);
...
test("namespace: detect namespace from pom.xml via ${appId} from properties", async (t) => {
	const {componentHInput} = t.context;
	componentHInput.configuration.resources.configuration.paths.src = "webapp-properties.appId";
