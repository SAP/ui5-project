...
test.serial("Installer: constructor requires 'endpointUrl' or ENV variable", (t) => {
	const {Installer} = t.context;
...
test.serial("Installer: fetchPackageVersions", async (t) => {
	...
	const repository = installer.getRepository();
	const requestMavenMetadataStub = sinon.stub(repository, "requestMavenMetadata")
		.resolves({
			versioning: {
				versions: {
