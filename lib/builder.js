 ...
 * @param {boolean} [parameters.archive=false] Whether to create an archive build
...
module.exports = async function({
	...
	selfContained = false, cssVariables = false, jsdoc = false, archive = false,
	...
	try {
		...
		await graph.traverseDepthFirst(async function({project, getDependencies}) {
			...
			const resources = await project.getReader({
				// Always use buildtime (=namespace) style when writing an archive
				style: archive ? "buildtime" : "runtime"
			}).byGlob("/**/*");

			log.verbose(`Writing out files...`);

			if (archive) {
				// Create and write archive metadata file
				const createArchiveMetadata = require("./buildHelpers/createArchiveMetadata");
				const metadata = await createArchiveMetadata(project, buildConfig);
				await fsTarget.write(resourceFactory.createResource({
					path: `/.ui5/archive-metadata.json`,
