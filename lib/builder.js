 ...
 * @param {boolean} [parameters.createBuildManifest=false] Whether to create a build metadata file
	...
	try {
		...
		await graph.traverseDepthFirst(async function({project, getDependencies}) {
			...
			const resources = await project.getReader({
				// Always use buildtime (=namespace) style when writing with a build manifest
				style: createBuildManifest ? "buildtime" : "runtime"
			}).byGlob("/**/*");
