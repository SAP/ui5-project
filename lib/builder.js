 ...
 * @param {boolean} [parameters.createBuildDescription=false] Whether to create a build metadata file
...
module.exports = async function({
	...
	selfContained = false, cssVariables = false, jsdoc = false, createBuildDescription = false,
	...
	try {
		...
		await graph.traverseBreadthFirst(async function({project, getDependencies}) {
			...
			if (projectsToBuild.includes(projectName) && builder.requiresDependencies(buildConfig)) {
				getDependencies().forEach((dep) => {
					...
					if (project.hasBuildDescription() && !dep.hasBuildDescription()) {
						throw new Error(
							`Project ${depName} must provide a build description since it is a dependency of ` +
							`project ${projectName} which already provides a build description`);
					}
		...
		projectsToBuild = projectsToBuild.filter((projectName) => {
			if (graph.getProject(projectName).hasBuildDescription()) {
				log.verbose(`Found a build description for project ${projectName}. Skipping build.`);
				return false;
			}
			return true;
		});
		...
		await graph.traverseDepthFirst(async function({project, getDependencies}) {
			...
			const resources = await project.getReader({
				// Always use buildtime (=namespace) style when writing an createBuildDescription
				style: createBuildDescription ? "buildtime" : "runtime"
			}).byGlob("/**/*");

			log.verbose(`Writing out files...`);

			if (createBuildDescription) {
				// Create and write createBuildDescription metadata file
				const createBuildDescription = require("./buildHelpers/createBuildDescription");
				const metadata = await createBuildDescription(project, buildConfig);
				await fsTarget.write(resourceFactory.createResource({
					path: `/.ui5/build-description.json`,
					string: JSON.stringify(metadata, null, "\t") // TODO 3.0: minify?
				}));
			}
