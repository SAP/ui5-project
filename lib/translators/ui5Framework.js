...
class FrameworkResolverOpenUI5 extends FrameworkResolver {
	...
	async _getLibraryMetadata(libraryName) {
		if (!this.metadata[libraryName]) {
			...
			const libraryMetadata = await this._registry.requestPackageManifest(pkgName, this._version);
			let dependencies = [];
			if (libraryMetadata.dependencies) {
				dependencies = Object.keys(libraryMetadata.dependencies).map(this._getLibaryName);
			}
			...
			if (libraryMetadata.devDependencies) {
				optionalDependencies = Object.keys(libraryMetadata.devDependencies).map(this._getLibaryName);
			}
