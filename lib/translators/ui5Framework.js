...
class FrameworkInstaller {
	constructor({dirPath, distVersion}) {
		...
		this._baseDir = path.join(homedir, ".ui5", "ui5_resources");
		this._cacheDir = path.join(homedir, ".ui5", "ui5_resources_cacache");
	...
	_getPacoteOptions() {
		...
		const userConfig = libnpmconfig.read({
		...
		log.verbose(`	@sap:registry: ${config["@sap:registry"]}`);
