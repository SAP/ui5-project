...
class FrameworkInstaller {
	constructor({registry, dirPath, distVersion}) {
		if (!dirPath) {
			throw new Error(`installer: missing parameter "dirPath"`);
		}
		this._baseDir = path.join(dirPath, ".ui5", "ui5_resources");
		log.verbose(`Installing to: ${this._baseDir}`);

		this._pacoteOptions = {
			registry: registry || "http://localhost:4873",
			cache: path.join(require("os").homedir(), ".ui5", "resources", "ui5_resources_cache")
		};
	...
	async _getDistMetadata() {
		...
		this._distMetadataCache = Promise.resolve().then(async () => {
			...
			log.info(`Using dist package in version ${version}...`);
