...
import projectGraphBuilder from "./graph/projectGraphBuilder.js";
import ui5Framework from "./graph/helpers/ui5Framework.js";
...
export async function graphFromPackageDependencies({
	...
	const {
		default: NpmProvider
	} = await import("./graph/providers/NodePackageDependencies.js");
...
export async function graphFromStaticFile({
	...
	const dependencyTree = await readDependencyConfigFile(cwd ? path.resolve(cwd) : process.cwd(), filePath);

	const {
		default: DependencyTreeProvider
	} = await import("./graph/providers/DependencyTree.js");
...
async function readDependencyConfigFile(cwd, filePath) {
	const {
		default: fs
...
if (process.env.NODE_ENV === "test") {
	graphFromStaticFile.readDependencyConfigFile = readDependencyConfigFile;
}
