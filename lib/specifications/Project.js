...
class Project extends Specification {
	...
	getReader({style = "buildtime"} = {}) {
		...
		// 	readers
		// });
		reader = this.getReaderForVersion(this.getCurrentWorkspaceVersion(), style);
	...
	*/
	getWorkspace() {
		...
		this.#latestWorkspace = createWorkspace({
			reader: this.getReaderForVersion(this.getCurrentWorkspaceVersion()),
	...
	// 		reader: this.getReaderForVersion(version),
	...
	getReaderForVersion(version, style = "buildtime") {
		if (version >= this.#writerVersions.length) {
			throw new Error(
				`Unknown resource version ${version}. The highest known version is ${this.#writerVersions.length - 1}`);
		} else if (version < 0) {
		...
		for (let i = version; i >= 0; i--) {
			this._addReadersFromWriter(style, readers, this.#writerVersions[i]);
		}
		readers.push(this._getStyledReader(style));

		return createReaderCollectionPrioritized({
			name: `Reader collection for project ${this.getName()} version ${version}`,
			readers: readers
		});
	}

	replaceWorkspaceForVersion(version) {
		if (version >= this.#writerVersions.length) {
			throw new Error(
				`Unknown resource version ${version}. The highest known version is ${this.#writerVersions.length - 1}`);
		}
		const writer = this._getWriter();
		this.#writerVersions[version] = writer;

		return createWorkspace({
			reader: this.getReaderForVersion(version - 1),
			writer: writer.collection || writer
		});
	}

	getCurrentWorkspaceVersion() {
		return this.#writerVersions.length - 1;
	}

	createNewWorkspaceVersion() {
		return this.#createNewWriterVersion();
	}
