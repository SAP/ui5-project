...
class Project extends Specification {
	 ...
	 * @param {object} [parameters.buildDescription] Build metadata object
	 */
	async init(parameters) {
		await super.init(parameters);

		const config = this._config;
		if (!this.__id.startsWith("@openui5/") && !this.__id.startsWith("@sapui5/")) {
		...
		this._buildDescription = parameters.buildDescription;

		await this._configureAndValidatePaths(config);
		await this._parseConfiguration(config, parameters.buildDescription);
	...
	getResourceTagCollection() {
		if (!this._resourceTagCollection) {
			...
			this._resourceTagCollection = new ResourceTagCollection({
				...
				tags: this.getBuildDescription()?.tags
			});
		}
