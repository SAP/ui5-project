	...
	*/
	async _parseConfiguration(config) {
		...
		if (!config.metadata.copyright) {
			try {
				config.metadata.copyrigh = await this.getCopyright();
			} catch (err) {
				// Catch error because copyright is optional
				// TODO: Make copyright mandatory?
				this._log.verbose(err.message);
			}
		}

		if (this.isFrameworkProject()) {
			if (config.builder && config.builder.libraryPreload && config.builder.libraryPreload.excludes) {
				this._log.verbose(
	...
	async _getCopyright() {
		...
		const {content: dotLibrary} = await this._getDotLibrary();
		if (dotLibrary && dotLibrary.library && dotLibrary.library.copyright) {
			this._log.verbose(
				`Using copyright from .library for project ${this.getName()}...`);
	...
	async _getManifest() {
		...
		return this._pManifest = this.getSourceReader().byGlob("**/manifest.json")
			.then(async (manifestResources) => {
				...
				try {
					...
					throw new Error(
						`Failed to read manifest.json for project ${this.getName()}: ${err.message}`);
				}
			});
	}
	...
	async _getDotLibrary() {
		...
		return this._pDotLibrary = this.getSourceReader().byGlob("**/.library")
	...
	async _getLibraryJsPath() {
		...
		return this._pLibraryJs = this.getSourceReader().byGlob("**/library.js")
