...
class Library extends ComponentProject {
	...
	*/
	async _parseConfiguration(config) {
		...
		if (config.customConfiguration?._archive) {
			this._namespace = config.customConfiguration._archive.namespace;
			return;
		}
		...
		if (!config.metadata.copyright) {
			try {
				config.metadata.copyright = await this._getCopyright();
			} catch (err) {
				// Catch error because copyright is optional
				// TODO: Make copyright mandatory?
				this._log.verbose(
					`Failed to get copyright for project ${this.getName()}: ${err.message}`);
			}
		}
