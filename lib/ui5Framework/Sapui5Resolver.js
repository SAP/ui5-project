...
class Sapui5Resolver extends AbstractResolver {
	...
	async handleLibrary(libraryName) {
		...
		const libraryMetadata = this._distMetadata.libraries[libraryName];
		...
		return {
			libraryMetadata: {
				id: libraryMetadata.npmPackageName,
				version: libraryMetadata.version,
				dependencies: libraryMetadata.dependencies,
				optionalDependencies: libraryMetadata.optionalDependencies
			},
			install
		};
	}
