...
const log = logger.getLogger("ui5Framework:AbstractResolver");
...
class AbstractResolver {
	 ...
	 *   Working directory to resolve project-configurations like .npmrc
	 * @param {string} [options.ui5HomeDir="~/.ui5"]
	 ...
	 * @param {string} [options.preferPreBuilt=false]
	 *   Whether to attempt to install pre-built versions of framework libraries
	 *   instead of sources.
	 */
	constructor({version, cwd, ui5HomeDir, preferPreBuilt = false}) {
		...
		this._preferPreBuilt = !!preferPreBuilt;
	}
		...
		const results = await Promise.all(libraryNames.map(async (libraryName) => {
			try {
				...
				if (sourceErrors.has(err.message)) {
					return `Failed to resolve library ${libraryName}:\n` +
						`See error logged above ("${err.message.substring(0, 30)}...")`;
				}
	...
	async install(libraryNames) {
		...
		if (errors.length === 1) {
			...
			// TODO: Transform to AggregateError
			// (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/AggregateError)
			// and handle in CLI Error-catch
