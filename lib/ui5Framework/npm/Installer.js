...
class Installer {
	...
	async installPackage({pkgName, version}) {
		const targetDir = this._getTargetDirForPackage({pkgName, version});
		const installed = await this._statPackage(targetDir);
		if (!installed) {
			await this._synchronize({pkgName, version}, async () => {
				// stat again to check whether package is now installed
				const installed = await this._statPackage(targetDir);
				if (!installed) {
					...
					log.info(`(Within lock) Alrady installed: ${pkgName} in version ${version}`);
				}
			});
		} else {
	...
	async _synchronize({pkgName, version}, callback) {
		...
		await Installer._mkdirp(this._lockDir);
		log.verbose("Locking " + lockPath);
		await Installer._lock(lockPath, {
			wait: 10000,
			stale: 60000,
			retries: 10
		});
		try {
			...
			await Installer._unlock(lockPath);
		}
	}
