		...
		if (!this._loadLibraryMetadata[libraryName]) {
			this._loadLibraryMetadata[libraryName] = Promise.resolve().then(async () => {
				...
				if (libraryManifest.dependencies) {
					dependencies = Object.keys(libraryManifest.dependencies).map(Openui5Resolver._getLibaryName);
				}
				...
				if (libraryManifest.devDependencies) {
					optionalDependencies = Object.keys(libraryManifest.devDependencies).map(Openui5Resolver._getLibaryName);
				}
