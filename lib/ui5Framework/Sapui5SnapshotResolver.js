...
const log = logger.getLogger("ui5Framework:Sapui5SnapshotResolver");
 ...
 * @public
 * @class
 * @alias @ui5/project/ui5Framework/Sapui5SnapshotResolver
 * @extends @ui5/project/ui5Framework/AbstractResolver
 */
class Sapui5SnapshotResolver extends AbstractResolver {
	/**
	...
	loadDistMetadata() {
		if (!this._loadDistMetadata) {
			this._loadDistMetadata = Promise.resolve().then(async () => {
				...
				const {artifactPath: versionJsonPath} = await this._installer.installArtifact({
					groupId: DIST_GROUP_ID,
					artifactId: DIST_ARTIFACT_ID,
					version,
					classifier: "sap-ui-version",
					extension: "json"
				});

				const metadata = await this._installer.readJson(path.join(distPkgPath, "metadata.json"));
				const versionJson = await this._installer.readJson(versionJsonPath);

				// Enrich metadata with GAV from sap-ui-version.json
				// TODO: Add GAV to metadata.json?
				versionJson.libraries.forEach((library) => {
					if (!library.npmPackageName) {
						// Ignore libraries that do not have an npm package name
						return;
					}
					if (!metadata.libraries[library.name]) {
						throw new Error(
							`Could not find library ${library.name}, listed in sap-ui-version.json, ` +
							`in corresponding distribution metadata.json of version ${version}`);
					}
					metadata.libraries[library.name].gav = library.gav;
				});
...
export default Sapui5SnapshotResolver;
