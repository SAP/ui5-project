...
class Sapui5MavenSnapshotResolver extends AbstractResolver {
	 ...
	 * @param {string} [options.cacheMode="eager"]
	...
	async handleLibrary(libraryName) {
		...
		return {
			...
			install: this._installer.installPackage({
				...
				artifactId: gav[1],
				version: metadata.version,
				classifier: this._preferPreBuilt ? null : "npm-sources",
				extension: this._preferPreBuilt ? "jar" : "zip"
			})
		};
	}
	static async fetchAllVersions({ui5HomeDir, cwd, endpointUrl} = {}) {
