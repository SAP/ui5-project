...
class Sapui5MavenSnapshotResolver extends AbstractResolver {
	...
	* @param {string} [options.cacheMode="default"] Can be "default" (cache everything, invalidate after 9 hours),
	 ...
	 * @param {string} [options.artifactsDir="${ui5HomeDir}/framework/artifacts"]
	 * @param {string} [options.packagesDir="${ui5HomeDir}/framework/packages"]
	 * @param {string} [options.metadataDir="${ui5HomeDir}/framework/metadata"]
	 * @param {string} [options.stagingDir="${ui5HomeDir}/framework/staging"]
	 */
	constructor(options) {
		super(options);
