...
class Installer extends AbstractInstaller {
		...
		return this._synchronize("metadata-" + fsId, async () => {
			...
			if (timeSinceLastCheck > CACHE_TIME || this._cacheMode === "off") {
				...
				if (localMetadata.lastCheck === 0) {
					log.verbose(
						`Could not find metadata for artifact ${logId} in local cache`);
				} else {
					log.verbose(
				...
				await this._removeStaleRevisions(localMetadata, coordinates);
	...
	async _removeStaleRevision(logId, metadata, {groupId, artifactId, classifier, extension}) {
	...
	_getStagingPathForArtifact({groupId, artifactId, revision, classifier, extension}) {
		if (!classifier) {
			classifier = revision;
			revision = "";
		}
		return path.join(this._stagingDir,
			groupId.replaceAll(".", "_"), artifactId, revision, `${classifier}.${extension}`);
	}
