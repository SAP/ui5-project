...
const log = logger.getLogger("ui5Framework:maven:Repository");

class Repository {
	constructor({cwd, endpointUrl}) {
		if (!cwd) {
			throw new Error(`Repository: Missing parameter "cwd"`);
		}
		if (!endpointUrl) {
			throw new Error(`Repository: Missing parameter "endpointUrl"`);
		}
		this._cwd = cwd;
	...
	async requestMavenMetadata({groupId, artifactId, version}) {
		try {
			...
			if (err.code === "ENOTFOUND") {
				throw new Error(this._getNoConnectionMessage(), {cause: err});
			}
			throw new Error(
	...
	_getNoConnectionMessage() {
		return `Failed to connect to Maven Repository at ${this._endpointUrl}. ` +
			`Please check the correct Endpoint URL is maintained and can be reached. `+
			`You may be able to continue working offline. For this, use the --offline flag`;
	}
}
