...
const log = logger.getLogger("ui5Framework:maven:Registry");

class Registry {
	constructor({cwd, cacheDir, endpointUrl}) {
		this._cwd = cwd;
		this._endpointUrl = endpointUrl;
		this._fetch = _fetch.defaults({
			cachePath: cacheDir
		});
	}

	/*
		version is optional to allow fetching the metadata file listing all versions of an artifact
	*/
	/**
	...
	async requestArtifact({groupId, artifactId, version, deploymentVersion, classifier, extension, targetPath}) {
		try {
			...
			const url = this._endpointUrl +
				...
				`${artifactId}-${deploymentVersion}${optionalClassifier}.${extension}`;
