...
import ProjectBuildCache from "./ProjectBuildCache.js";
...
class ProjectBuildContext {
	constructor({buildContext, project}) {
		...
		this._cache = new ProjectBuildCache(this._project);
		this._queues = {
			cleanup: []
		};
	...
	getCacheKey() {
		return createCacheKey(this._project,
			this._buildContext.getBuildConfig(), this._buildContext.getTaskRepository());
	}
	...
	async watchFileChanges() {
		...
		watcher.on("change", (filePath) => {
			...
			// Inform dependents
			this._buildContext.getGraph().getTransitiveDependents(this._project.getName()).forEach((dependent) => {
				this._buildContext.getProjectBuildContext(dependent).dependencyFileChanged(resourcePath);
			});
		});
	}
