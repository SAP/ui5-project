...
class WatchHandler extends EventEmitter {
	...
	async #fileChanged(project, filePath) {
		// const changedResources = [resourcePath];

		// while(changedResources.length) {
		// 	const changedResource = changedResources.pop();
		// for (const context of this.#projectBuildContexts) {
		// }
		// const impliedChangedResources = await context.getBuildCache().fileChanged(changedResource);
		// changedResources.push(...impliedChangedResources);

		// }
		// for (const context of this.#projectBuildContexts) {
		// 	await context.getBuildCache().fileChanged(filePath);
		// }
	...
	// async startWatching() {
	// 	// First callback, start watching relevant files
	// 	for (const projectBuildContext of this._projectBuildContexts) {
	// 		await projectBuildContext.getBuildCache().onSourceFileChange(this.fileChanged.bind(this));
	// 	}
	// }
	...
	async #handleResourceChanges() {
		...
		// for (const [projectBuildContext, resourcePaths] of changes) {
		// }

		// Trigger callbacks
		// for (const callback of this.#sourceFileChangeHandlers) {
		// 	await callback({
		// 		changes,
		// 	});
		// }
	...
	handleResourceChangeInRunningBuild(projectName, changedResourcePaths) {
		const graph = this.#buildContext.getGraph();
		const emptySet = new Set();

		// Propagate changes to dependents of the project
		for (const {project: dep} of graph.traverseDependents(projectName)) {
			const projectBuildContext = this.#buildContext.getBuildContext(dep.getName());
			projectBuildContext.getBuildCache().resourceChanged(emptySet, changedResourcePaths);
		}
	}

	// async #handleProjectResourceChanges(projectBuildContext, resourcePaths) {
	// 	// Inform all relevant project build caches
	// 	const changedResources = new Set(resourcePaths);
	// 	const impliedChangedResources = await projectBuildContext.getBuildCache().resourceChanged(changedResources);
	// 	for (const impliedChangedResource of impliedChangedResources) {
	// 		changedResources.add(impliedChangedResource);
	// 	}

	// 	const dependents = this.#buildContext.getGraph().getTransitiveDependents(
	// 		projectBuildContext.getProject().getName());
	// 	for (const dependent of dependents) {
	// 		const dependentContext = this.#buildContext.getBuildContext(dependent);
	// 		if (!dependentContext) {
	// 			throw new Error(`Could not find dependent project context for project ${dependent}`);
	// 		}
	// 		const impliedChangedResources = dependentContext.getBuildCache().resourceChanged(changedResources);
	// 		for (const impliedChangedResource of impliedChangedResources) {
	// 			changedResources.add(impliedChangedResource);
	// 		}
	// 	}
