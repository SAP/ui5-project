...
export default class ProjectBuildCache {
	...
	fileChanged(resourcePath) {
		for (const {taskName, resourcesRead} of this.#versions.values()) {
			if (resourcesRead.has(resourcePath)) {
				log.info(`Invalidating build task ${taskName} of project ${this.#project.getName()} ` +
					`due to change in file ${resourcePath}`);
			}
		}
	}
