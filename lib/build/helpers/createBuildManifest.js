...
export default async function(project, buildConfig, taskRepository, transitiveDependencyInfo, buildCache) {
	if (!project) {
	...
	if (!transitiveDependencyInfo) {
		throw new Error(`Missing parameter 'transitiveDependencyInfo'`);
	}
	if (!buildCache) {
	...
	if (project.isFrameworkProject()) {
		...
		buildManifest = {
			...
			dependencies: transitiveDependencyInfo,
			version: project.getVersion(),
			namespace: project.getNamespace(),
			tags: getSortedTags(project),
			cacheKey: createCacheKey(project, buildConfig, taskRepository),
		};
	}
	...
	const metadata = {
		...
		buildManifest,
		buildCache: buildCache.serialize(),
	};
...
export async function createCacheKey(project, buildConfig, taskRepository) {
	const {builderVersion, fsVersion: builderFsVersion} = await taskRepository.getVersions();
	const projectVersion = await getVersion("@ui5/project");
	const fsVersion = await getVersion("@ui5/fs");

	const cacheKey = `${project.getName()}-${project.getVersion()}-` +
		`${builderVersion}-${projectVersion}-${fsVersion}-${builderFsVersion}-${JSON.stringify(buildConfig)}`;
