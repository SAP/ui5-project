...
class TaskRunner {
	...
	static async create({graph, project, parentLogger, taskUtil, taskRepository, buildConfig}) {
		...
		taskRunner._project = project;
		taskRunner._graph = graph;
		taskRunner._taskUtil = taskUtil;
		taskRunner._taskRepository = taskRepository;
		taskRunner._buildConfig = buildConfig;

		taskRunner._log = parentLogger.createSubLogger(project.getType() + " " + project.getName(), 0.2);
		taskRunner._taskLog = taskRunner._log.createTaskLogger("🔨");

		taskRunner._tasks = {};
		taskRunner._taskExecutionOrder = [];
		...
		for (const [taskName, params] of standardTasks) {
			taskRunner._addTask(taskName, params);
		}

		taskRunner._addCustomTasks({
			graph,
			project,
			taskUtil
		});
