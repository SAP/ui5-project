...
import {createReaderCollection, createTrace} from "@ui5/fs/resourceFactory";
...
class TaskRunner {
	...
	async runTasks() {
		...
		this._cache.setTasks(allTasks);
		for (const taskName of allTasks) {
			if (this._cache.hasValidCacheForTask(taskName)) {
				this._log.info(`Task ${taskName} for project ${this._project.getName()} is cached`);
				continue;
			}
	...
	async runTasksForResources(resourcePaths) {
	}

	async runTasksForChangedResources(changedResources) {
	}
	...
	_addTask(taskName, {requiresDependencies = false, options = {}, taskFunction} = {}) {
		...
		if (taskFunction === null) {
			...
			task = async (log) => {
				...
				// TODO: Same for custom tasks
				this._project.createNewWorkspaceVersion();
				this._log.info(
					`Executing task ${taskName} for project ${this._project.getName()} with workspace version ` +
					`${this._project.getCurrentWorkspaceVersion()}`);
				const workspace = createTrace(this._project.getWorkspace());
				const params = {
					workspace,
					taskUtil: this._taskUtil,
					options
				...
				if (requiresDependencies) {
					dependencies = createTrace(this._allDependenciesReader);
				...
				this._cache.addLevel(taskName, this._project.getCurrentWorkspaceVersion(), workspace, dependencies);
			};
		}
