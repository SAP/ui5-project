...
export default class BuildTaskCache {
	...
	checkPossiblyInvalidatesTask(projectResourcePaths, dependencyResourcePaths) {
		if (this.#isRelevantResourceChange(this.#projectRequests, projectResourcePaths)) {
			log.verbose(
				`Build task cache invalidated for task ${this.#taskName} of project ${this.#projectName}. Cause:`);
			log.verbose(projectResourcePaths);
			return true;
		}
		...
		if (this.#isRelevantResourceChange(this.#dependencyRequests, dependencyResourcePaths)) {
			log.verbose(
				`Build task cache invalidated for task ${this.#taskName} of project ${this.#projectName}. Cause:`);
			log.verbose(dependencyResourcePaths);
			return true;
		}
	...
	async isResourceInReadCache(resource) {
		...
		if (cachedResource.hash) {
			return this.#isResourceFingerprintRequal(resource, cachedResource);
		} else {
	...
	async #isResourceEqual(resourceA, resourceB) {
		...
		if (resourceA === resourceB) {
			log.info(`Resource ${resourceA.getPath()} IS THE SAME ==========================================`);
			return true;
		}
	...
	async #isResourceFingerprintRequal(resourceA, resourceBMetadata) {
