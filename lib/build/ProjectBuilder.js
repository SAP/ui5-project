...
class ProjectBuilder {
	...
	async #update(projectBuildContexts, requestedProjects, fsTarget, cacheDir) {
		...
		for (const projectBuildContext of queue) {
			...
			this.#log.verbose(`Updating target workspace...`);
			if (fsTarget) {
				await this._writeResults(projectBuildContext, fsTarget);
			}

			if (cacheDir) {
				await projectBuildContext.getBuildCache().serializeToDisk();
			}
		}
	}
