...
class ProjectBuilder {
	...
	async build({
		...
		const {projectBuildContexts, buildDependencies} = await this._createRequiredBuildContexts(requestedProjects);
		...
		await this._graph.traverseDepthFirst(({project}) => {
			...
			if (projectBuildContext) {
				...
				if (!currentQueueEntry) {
					// Create new entry
					currentQueueEntry = new Map();
					currentQueueEntry.set(projectName, projectBuildContext);
					queue.push(currentQueueEntry);
				} else {
					// Check current entry and append project if no dependency exists
					const buildDeps = buildDependencies.get(projectName);
					const entryContainsDep = Array.from(currentQueueEntry.keys()).some((depName) => {
						return buildDeps.has(depName);
					});
					if (entryContainsDep) {
						// Create a new entry
						currentQueueEntry = new Map();
						...
						// Append project to current entry
						currentQueueEntry.set(projectName, projectBuildContext);
					}
				}
		...
		for (const queueEntry of queue) {
			for (const projectBuildContext of queueEntry.values()) {
				projectNames.push(projectBuildContext.getProject().getName());
			}
		}
		log.setProjects(projectNames);
		if (queue.length > 1) { // Do not log if only the root project is being built
			log.info(`Processing ${projectNames.length} projects`);
			if (alreadyBuilt.length) {
				log.info(`  Reusing build results of ${alreadyBuilt.length} projects`);
				log.info(`  Building ${projectNames.length - alreadyBuilt.length} projects`);
			}
		...
		try {
			const pWrites = [];
			for (const queueEntry of queue) {
				const projectBuildContexts = Array.from(queueEntry.values());
	...
	async _createRequiredBuildContexts(requestedProjects) {
		...
		for (const projectName of requiredProjects) {
			...
			if (projectBuildContext.requiresBuild()) {
				...
				buildDependencies.set(projectName, requiredDependencies);
		...
		return {projectBuildContexts, buildDependencies};
	}
