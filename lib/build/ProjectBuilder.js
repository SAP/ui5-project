...
class ProjectBuilder {
	...
	async build({
		...
		const target = targetWriter ?? resourceFactory.createAdapter({
			fsBasePath: destPath,
			virBasePath: "/"
		});
		...
		try {
			...
			for (const projectBuildContext of queue) {
				...
				pWrites.push(this._writeResults(projectBuildContext, target));
	...
	async _writeResults(projectBuildContext, target) {
		...
		if (createBuildManifest) {
			...
			for (const depName of this._graph.getTransitiveDependencies(project.getName())) {
				const dep = this._graph.getProject(depName);
				transitiveDependencyInfo[depName] = {
					version: dep.getVersion(),
					cacheKey: dep.getCacheKey(),
				};
			}
			const {
			...
			const metadata = await createBuildManifest(
				...
				transitiveDependencyInfo, projectBuildContext.getBuildCache());
