...
class AbstractBuilder {
	...
	build(buildConfig, buildParams) {
		...
		return allTasks.reduce((taskChain, taskName) => {
			...
			if (typeof taskFunction === "function") {
				taskChain = taskChain.then(this.wrapTask(taskName, taskFunction, buildParams));
			}
	...
	requiresDependencies(buildConfig) {
		...
		return allTasks.some((taskName) => {
			if (this.tasks[taskName].requiresDependencies) {
				this.log.info(`Task ${taskName} for project ${this.project.getName()} requires dependencies`);
				return true;
			}
			return false;
		});
	 ...
	 */
	wrapTask(taskName, taskFunction, taskParams) {
		return () => {
			this.taskLog.startWork(`Running task ${taskName}...`);
			return taskFunction(taskParams).then(() => this.taskLog.completeWork(1));
		};
	}
