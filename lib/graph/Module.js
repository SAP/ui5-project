	...
	*/
	async _getConfigurations() {
		...
		if (!configurations || !configurations.length) {
			configurations = await this._getArchiveConfigurations();
		}
		if (!configurations || !configurations.length) {
	...
	async _getArchiveConfigurations() {
		const config = await this._readArchiveMetadata();

		if (!config) {
			log.verbose(`Could not find any archive metadata files in module ${this.getId()}`);
			return [];
		}
	...
	async _readArchiveMetadata() {
		const reader = await this.getReader();
		const archiveMetadataResource = await reader.byPath("/.ui5/archive-metadata.json");
		if (archiveMetadataResource) {
			return JSON.parse(await archiveMetadataResource.getString());
		}
	}
