...
const SAP_LEGACY_LIBRARIES = ["sap.ui.core"];
	...
	async _getSpecifications() {
		...
		configs.forEach((configuration) => {
			if (configuration.kind === "project" && configuration.type === "library" &&
				...
				// Old legacy-libraries where configured as type "library"
				if (SAP_LEGACY_LIBRARIES.includes(libraryName)) {
					configuration.type = "legacy-library";
				}
			}
		});

		const specs = await Promise.all(configs.map(async (configuration) => {
			const buildDescription = configuration._buildDescription;
			if (configuration._buildDescription) {
				delete configuration._buildDescription;
			}
	...
	*/
	async _getConfigurations() {
		...
		if (!configurations || !configurations.length) {
			configurations = await this._getBuildDescriptionConfigurations();
		}
		if (!configurations || !configurations.length) {
	...
	async _getSuppliedConfigurations() {
		if (this._suppliedConfigs.length) {
			...
			return await Promise.all(this._suppliedConfigs.map(async (suppliedConfig) => {
				...
				if (suppliedConfig.buildDescription) {
					config = suppliedConfig.project;
					config._buildDescription = suppliedConfig.buildDescription;
				}
	...
	async _getBuildDescriptionConfigurations() {
		const buildDescriptionMetadata = await this._readBuildDescription();

		if (!buildDescriptionMetadata) {
			log.verbose(`Could not find any build description in module ${this.getId()}`);
			return [];
		}

		const config = buildDescriptionMetadata.project;
		config._buildDescription = buildDescriptionMetadata.buildDescription;
		return [this._normalizeAndApplyShims(config)];
	}

	async _readBuildDescription() {
		const reader = await this.getReader();
		const buildDescriptionResource = await reader.byPath("/.ui5/build-description.json");
		if (buildDescriptionResource) {
			return JSON.parse(await buildDescriptionResource.getString());
		}
	}
