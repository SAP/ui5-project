...
import {globby} from "globby"; // TODO: Add dependency to package.json
...
class Workspace {
	...
	async _getModulesFromPath(cwd, relPath, resolvePackageWorkspace = true) {
		...
		if (packageWorkspaceConfig?.length) {
			...
			const matchedPaths = await globby(packageWorkspaceConfig, {
				cwd: nodePath,
				onlyDirectories: true,
				followSymbolicLinks: false
			});

			return Promise.all(matchedPaths.map(async (pkgPath) => {
				const nodes = await this._getModulesFromPath(nodePath, pkgPath, false);
				if (nodes.lengh > 1) {
					throw new Error(
