...
import {fromFile as getConfigFromFile} from "../config/Configuration.js";

let resolverConfig;
async function getReolverConfig() {
	return Promise.resolve().then(async () => {
		if (!resolverConfig) {
			resolverConfig = await getConfigFromFile();
		}
...
export async function graphFromPackageDependencies({
	...
	if (resolveFrameworkDependencies) {
		const configuration = await getReolverConfig();
		await ui5Framework.enrichProjectGraph(projectGraph, {versionOverride, workspace, configuration});
	}
...
export async function graphFromStaticFile({
	...
	if (resolveFrameworkDependencies) {
		const configuration = await getReolverConfig();
		await ui5Framework.enrichProjectGraph(projectGraph, {versionOverride, configuration});
	}
