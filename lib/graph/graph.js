...
import {fromFile as getResolverConfigFromFile} from "../config/Configuration.js";
...
export async function graphFromPackageDependencies({
	...
	if (resolveFrameworkDependencies) {
		const resolverConfig = await getResolverConfigFromFile();
		await ui5Framework.enrichProjectGraph(projectGraph, {versionOverride, workspace, resolverConfig});
	}
...
export async function graphFromStaticFile({
	...
	if (resolveFrameworkDependencies) {
		const resolverConfig = await getResolverConfigFromFile();
		await ui5Framework.enrichProjectGraph(projectGraph, {versionOverride, resolverConfig});
	}
