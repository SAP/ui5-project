...
export async function graphFromPackageDependencies({
	...
	if (workspaceConfiguration) {
		...
		if (workspaceConfiguration.metadata.name !== activeWorkspace) {
			...
			log.verbose(`Using provided workspace configuration ${workspaceConfiguration.metadata.name}...`);
		...
		if (workspaceConfiguration) {
			log.verbose(`Using workspace configuration "${activeWorkspace}" from ${workspaceConfigPath}...`);
			workspace = new Workspace({
				cwd,
				workspaceConfiguration
			});
