...
export async function graphFromPackageDependencies({
	...
	if (resolveFrameworkDependencies) {
		const config = await getResolverConfigFromFile();
		await ui5Framework.enrichProjectGraph(projectGraph, {versionOverride, workspace, config});
	}
...
export async function graphFromStaticFile({
	...
	if (resolveFrameworkDependencies) {
		const config = await getResolverConfigFromFile();
		await ui5Framework.enrichProjectGraph(projectGraph, {versionOverride, config});
	}
