...
export default {
	 ...
	 * @param {@ui5/project/config/Configuration} [options.resolverConfig]
	 *   Optional configuration file to use for the resolvers
	...
	enrichProjectGraph: async function(projectGraph, options = {}) {
		...
		if (rootProject.isFrameworkProject() && (!version || !version.endsWith("-SNAPSHOT"))) {
			// If the root project is a framework project, and the framework version is not a snapshot version,
		...
		const config = options.configuration || {cwd: rootProject.getRootPath(), version};
		const resolver = new Resolver({...config, providedLibraryMetadata});
