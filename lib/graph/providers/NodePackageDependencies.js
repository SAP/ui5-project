...
class NodePackageDependencies {
	 ...
	 */
	constructor({cwd, rootConfiguration, rootConfigPath, workspace}) {
		this._cwd = cwd;
	...
	async _resolveModulePath(baseDir, moduleName) {
		...
		if (this._workspace) {
			...
			const workspaceNode = await this._workspace.getNode(moduleName);
			if (workspaceNode) {
				log.verbose(`Resolved module ${moduleName} via workspace ${this._workspace.getName()} ` +
					`to path ${workspaceNode.path}`);
				return workspaceNode.path;
			}
		}
