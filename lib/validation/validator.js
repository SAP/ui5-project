...
/**
 * Validates the given ui5 configuration and returns default values if not provided.
 *
 * @public
 * @function
 * @static
 ...
 * @param {module:@ui5/builder/processors/bundlers/moduleBundler~ModuleBundleDefinition} bundleOptions
 *   BundleOptions object to be enhanced
 ...
 */
export async function enhanceBundleWithDefaults(bundleDefinition, bundleOptions, project) {
	const config = {
		specVersion: `${project.getSpecVersion()}`,
		type: `${project.getType()}`,
		metadata: {name: project.getName()},
		builder: {bundles: [{bundleDefinition, bundleOptions}]}
	};
	const result = await getDefaults({config, project: {id: project.getName()}});

	return result.config.builder.bundles[0];
}
