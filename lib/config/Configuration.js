...
export async function resolveSnapshotEndpointUrl() {
	const {default: yesno} = await import("yesno");
	const ok = await yesno({
		question: "SnapshotURL is missing. " +
			"Do you want to try to resolve it from ~/.m2/settings.xml? (yes)",
		defaultValue: true
	});

	const settingsXML = path.join(os.homedir(), ".m2", "settings.xml");
	const {default: fs} = await import("graceful-fs");
	const {promisify} = await import("node:util");
	const readFile = promisify(fs.readFile);
